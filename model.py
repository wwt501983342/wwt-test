...
class MHSA_Intra(nn.Module):
    ...
    def forward(self, input, intra_attn_mask):
        ...
        value = self.conv_key(input).view(B, self.heads, self.dim_head, T).permute(0, 1, 3, 2).contiguous() #(B, h, T, dim_head)
...
class ASMLoc_Base(nn.Module):
    """
    ASMLoc_Base: base model wwithout action-aare segment modules
    """
...
class ASMLoc(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.uncertainty_branch.weight.data.normal_(0.0, 0.0001)## m.weight.data是卷积核参数, m.bias.data是偏置项参数
    ...
    def forward(self, input_feature, proposal_bbox, proposal_count_by_video, vid_len, vid_name):
        ...
        input_feature = input_feature.permute(0, 2, 1)
        embeded_feature = self.feature_embedding(input_feature)  #(B, C, T)（16，2048，750）
