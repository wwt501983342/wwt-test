...
def test(args, model, dataloader, criterion, logger=None, step=0):
    ...
    for vid_name, input_feature, vid_label, vid_len, proposal_bbox, proposal_count_by_video, pseudo_instance_label, dynamic_segment_weights_cumsum in tqdm(dataloader):
        ...
        cas_thresh_list = np.arange(0.15, 0.25, 0.05)
        att_thresh_list = np.arange(0.1, 0.925, 0.025)
        proposal_dict = {}
        ...
        for cas_thresh in cas_thresh_list:
            ...
            props_list = get_proposal_oic(tmp_seg_list, (1.0*tmp_int_cas + 0.0*int_temp_att_fg_score_np), cls_prediction, \
                    score_np, t_factor, lamb=0.2, gamma=0.0, dynamic_segment_weights_cumsum=dynamic_segment_weights_cumsum[0], vid_duration=vid_duration)
    ...
    anet_detection = ANETDetection(ground_truth_file=args.test_gt_file_path,
                    ...
                    subset="test", dataset='THUMOS', logger=logger)
...
def generate_pseudo_segment(args, model, dataloader, step=0):
    ...
    att_thresh_list = np.arange(0.4, 0.925, 0.025)
    for vid_name, input_feature, vid_label, vid_len, proposal_bbox, proposal_count_by_video, pseudo_instance_label, dynamic_segment_weights_cumsum in tqdm(dataloader):
        input_feature = input_feature.to(device)
        vid_label = vid_label.to(device)
        proposal_bbox = proposal_bbox.to(device)
...
def generate_dynamic_segment_weights(args, pseudo_segment_dict, step=0):
    ...
    for vid_name in pseudo_segment_dict['results']:
        if 'test' in vid_name:
            feature = np.load(os.path.join('data/THUMOS14/{}'.format('test'), vid_name+".npy"))
        elif 'validation' in vid_name:
            feature = np.load(os.path.join('data/THUMOS14/{}'.format('train'), vid_name+".npy"))
        ...
        for label in label_set:
            ...
            prediction_list = sorted(prediction_list, key=lambda k: k[2], reverse=True)#降序，按置信度排列
        ...
        upscale_duration = args.gamma * (2 * args.delta + 1)##常数12
...
def main(args):
    torch.set_printoptions(precision=4)#浮点数组输出的精度为4
    ...
    if args.checkpoint or args.test:#如果checkpoint或test参数存在
        checkpoint_path = args.checkpoint#则设定path为checkpoint参数
    elif not args.no_resume:#如果不是no_resume（正常训练），就设定path为latest.pth
    ...
    best_test_mAP = 0
    step = 0
    if checkpoint_path and os.path.isfile(checkpoint_path):#如果path存在且path里有文件
        print("load checkpoint from {}".format(checkpoint_path))
        checkpoint = torch.load(checkpoint_path, map_location=device)#加载checkpoint
        ...
        with torch.no_grad():
            for step in range(args.num_steps+1):
                ...
                if step < args.num_steps:
                    ...
                    tcheckpoin = torch.load(os.path.join(checkpoint_path, "model_step{}_last_epoch.pth".format(step)), map_location=device)
                else:
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        torch.cuda.manual_seed(args.seed)#为GPU设置种子
        torch.cuda.manual_seed_all(args.seed)#为所有GPU设置种子
    torch.backends.cudnn.benchmark = False#如果是TRUE的话，提升训练速度，程序开始时花费额外时间搜索最适合的卷积算法
    torch.backends.cudnn.deterministic = True#固定cuda的随机数种子，每次卷积的结果是确定的
    main(args)