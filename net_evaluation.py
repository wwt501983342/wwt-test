...
def temporal_interpolation(temp_data, output_length):
    ...
    int_temp_x = np.linspace(0, temp_len-1, output_length)#在0，temp_len-1长度上，均匀750个点
    ...
    for i in range(len(tempseg_list)):
        ...
        if temp_list.any():
            ...
            for j in range(len(grouped_temp_list)):
                ...
                c_score = inner_score - outer_score + gamma * c_pred_scores[c_pred[i]]##gamma这里为0
...
def get_segment_iou(target_segment, candidate_segment):
    ...
    tIOU = segment_intersection.astype(float) / segment_union    
...
def nms(proposals, thresh):
    ...
    areas = x2 - x1 + 1
    order = scores.argsort()[::-1]#-1表示降序排列，返回索引值
    keep = []
    while order.size > 0:
        i = order[0]
        ...
        inds = np.where(iou < thresh)[0]#这里的[0]表示返回的是行索引
        order = order[inds + 1]
